...
{
    memcpy(&gLogArgs, (void*)args, sizeof(struct LogArgs));
    if(gLogArgs.logfilePath)
    {
        gLogFile = fopen(gLogArgs.logfilePath, "w");
    }
}
...
int vLog_Fmt(const char* fmt, enum LogLvl lvl, va_list args)
{
    ...
    char** levelNames = gLogArgs.bLogTextColoured ? gColouredLogLevelNames : gLogLevelNames;
    snprintf(gLogBuffer, 512, levelNames[lvl]);
    int namelen = strlen(levelNames[lvl]);
    char* start = gLogBuffer + namelen;
    if(gLogArgs.bIncludeLogTimeStamps)
    {
        const char* timeFmtString = gLogArgs.bLogTextColoured ? UWHT"%02d:%02d:%02d"CRESET" " : "%02d:%02d:%02d ";
        time_t rawtime;
        struct tm *info;
        time( &rawtime );
