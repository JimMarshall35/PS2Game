#include "IDTEX8_CSM1_CLUTSwizzleTable.h"

// A LUT to translate a naive colour lookup table index to the swizzled format the ps2 needs
// For the 8 bit index format (IDTEX8) using the more efficient CSM1 CLUT storage mode
// See GS User Manual page 30
unsigned char CLUT_IDTEX8_CSM1_SwizzleTable[256] = {
, 
};
